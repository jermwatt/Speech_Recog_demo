...
class Fit_Bases:
    ...
    def plot_approx(self,clf,domain,transform):
        ...
        # plot regressor
        plt.plot(domain,z,linewidth = 3,color = 'k')
    ...
    def make_cos_approx(self,signal,num_elements):
        ...
        for n in np.arange(num_elements):
            if np.mod(n,2) == 0:
                f.append(np.cos(2*np.pi*n*signal))
            else:
                f.append(np.sin(2*np.pi*n*signal))
    ...
    def browse_cos_fit(self):
        ...
        interact(show_fit, num_elements=widgets.IntSlider(min=1,max=10,step=1,value=1))
